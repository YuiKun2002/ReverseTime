...
void UUI_TreviFountain2BoxSelect::Select_Item()
{
	...
	{
		//重新记录祈愿次数
		UUI_SelectTip* LSelect = UWidgetBase::CreateSelectTipWidget(
			FText::FromString(TEXT("是否选择取消保底自选锁定，如果取消被记录的次数将会重置为0")));
		//绑定选择
		FScriptDelegate Delegate;
		Delegate.BindUFunction(this, TEXT("Cancel_Item"));
		LSelect->BindOk(MoveTempIfPossible(Delegate));
	}
	else {
