...
{
	UUI_SettingButtonTab* LButtonTab = _Widget->CreateChildWidget<UUI_SettingButtonTab>(
		UGameSystemFunction::GetUserInterClassByName(
		MoveTemp(GameDataNameAsset::UI_Setting::Setting),
		MoveTemp(GameDataNameAsset::UI_Setting::SettingButtonTab)
	));

	UWidgetBase* LWidget = _Widget->CreateChildWidget<UWidgetBase>(
		LoadClass<UWidgetBase>(0,
			*_Datas[_Index].M_TargetBlueprintRefPath)
	);
