...
void ABoss_DaLiShen::Update_Attack_1(float DeltaTime)
{
	...
	int32 M_RowCount = MesheCtrl->GetMapMeshRowAndCol().Row;
	int32 M_ColCount = MesheCtrl->GetMapMeshRowAndCol().Col;
	...
	if (this->Attack_1_FristDelay <= 0.f)
	{
		if (!this->bAttack_1_Frist)
		{
			...
			LocalObj->SetTarget(MesheCtrl->GetMapMeshLocation(SRandRow, SRandCol));
...
void ABoss_DaLiShen::GameTick(float DeltaTime)
{
