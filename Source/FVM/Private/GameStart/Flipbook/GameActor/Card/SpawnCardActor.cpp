...
void ASpawnCardActor::OnCardInitData(const FItemCard& NewBaseData)
{
	//初始化父类【以及组件】
	Super::OnCardInitData(NewBaseData);

	//初始化必要数据
	this->ItemCardSpawnData = this->GetCardDataObject()->GetCardSpawnData(this->CardID);
	this->ItemCardSpawnData.Level = this->GetCardSourceData().Level;
	...
	//赋值基础属性
	this->SetCardData(this->ItemCardSpawnData);
}