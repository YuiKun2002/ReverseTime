...
void UUI_CardCommand::EradicateCard()
{
	if (this->M_MapMeshe)
	{
		...
		AGameActorFlipbookBase* Eradicate_ =
			this->GetWorld()->SpawnActor<AGameActorFlipbookBase>(
				LoadClass<AGameActorFlipbookBase>(0,
					TEXT("Blueprint'/Game/Resource/BP/GameStart/VS/BPEradicate.BPEradicate_C'")),
				this->M_MapMeshe->GetUIMapMesheTransform());
...
void UUI_CardCommand::RemoveCurrentPanel()
{
	if (this->M_ACreateCardBar)
	{
		...
		AGameMapInstance::GetGameMapCardManagerStatic()->ShowCardBar();
	}
}
