...
void UGameMapUI_EditorTab::EditAlienName(const FString& SourceName, const FString& TargetName)
{
	...
	UDataTable* Map = LoadObject<UDataTable>(nullptr,
		TEXT("DataTable'/Game/Resource/BP/Data/MapData/DT_GameMapDataList.DT_GameMapDataList'"));
	UDataTable* Mouse = LoadObject<UDataTable>(nullptr,
		TEXT("DataTable'/Game/Resource/BP/Data/MouseData/MouseData.MouseData'"));

	TArray<FMouse_Data*> MouseList;
	Mouse->GetAllRows<FMouse_Data>(TEXT("AlienLists"), MouseList);
	for (FMouse_Data* list : MouseList)
	{
	...
	UPackage* PackageA = FindPackage(nullptr,
		*FPackageName::FilenameToLongPackageName(Mouse->GetPathName())
	);
	if (PackageA)
	{
