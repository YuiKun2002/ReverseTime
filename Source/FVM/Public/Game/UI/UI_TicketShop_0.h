...
private:
	...
	//加载器
	UPROPERTY()
	UItemLoaderObject* M_Ticket_UItemLoaderObject = nullptr;
public:
	//创建材界面
	UWidget* WidgetCreate_Init(const class UItemLoadDataBlock* const _Data, int32 _Index);
	//刷新界面
	void WidgetRefresh_Update(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);
public:
	//剔除不显示的商品
