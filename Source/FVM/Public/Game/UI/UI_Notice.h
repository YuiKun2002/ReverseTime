...
private:
	//物品加载器
	UPROPERTY()
	UItemLoaderObject* M_UItemLoaderObject = nullptr;
	//是否初始化
	UPROPERTY()
	bool bInit = false;
private:
...
public:
	...
	//创建加载
	UWidget* BeginLoadList(const class UItemLoadDataBlock* const _item, int32 _index);
	//更新加载
	void UpdateList(const class UItemLoadDataBlock* const _item, int32 _Data_Index, UWidget* _Widget);
	//预更新
	void BeginUpdate();
