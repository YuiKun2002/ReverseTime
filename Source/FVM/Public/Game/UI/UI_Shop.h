...
private:
	//加载器(开始界面)
	UPROPERTY()
	UItemLoaderObject* M_UItemLoaderObject_0 = nullptr;
	UPROPERTY()
	UItemLoaderObject* M_UItemLoaderObject_1 = nullptr;
	//其他道具-加载器
	UPROPERTY()
	UItemLoaderObject* M_UItemLoaderObject_2 = nullptr;
private:
	//-------------------------------------服装区【角色形象界面】----------------------------------------
	UPROPERTY()//角色界面容器
...
public:
	//创建材界面
	UWidget* WidgetCreate_Init_0(const class UItemLoadDataBlock* const _Data, int32 _Index);
	//刷新界面
	void WidgetRefresh_Update_0(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);
	//创建材界面
	UWidget* WidgetCreate_Init_1(const class UItemLoadDataBlock* const _Data, int32 _Index);

	//初始化商品数据
	void InitShopGridData(const class UItemLoadDataBlock* const _Data, UUI_ShopGrid* _UWidget);
public:
	//数据表数据(抢购左-区域)
	UPROPERTY()
