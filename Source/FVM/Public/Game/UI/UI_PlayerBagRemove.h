...
private:
	//加载器
	UPROPERTY()
	UItemLoaderObject* M_UItemLoaderObject_Equipment = nullptr;
	//创建材界面
	UWidget* WidgetCreate_InitEquipment(const class UItemLoadDataBlock* const _Data, int32 L_Index);
	//刷新界面
	void WidgetRefresh_UpdateEquipment(const class UItemLoadDataBlock* const _Data, int32 L_Index, UWidget* _UWidget);

	//加载器
	UPROPERTY()
	UItemLoaderObject* M_UItemLoaderObject_Cards = nullptr;
	//创建材界面
	UWidget* WidgetCreate_InitCards(const class UItemLoadDataBlock* const _Data, int32 L_Index);
	//刷新界面
	void WidgetRefresh_UpdateCards(const class UItemLoadDataBlock* const _Data, int32 L_Index, UWidget* _UWidget);
public:
	//初始化加载器
	UFUNCTION()
