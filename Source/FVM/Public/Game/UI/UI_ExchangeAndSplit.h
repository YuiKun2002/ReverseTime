...
public:
	...
	//设置当前选择的分解物品对象
	void SetCurrentSlipItem(
		const EItemType& _CurrentSelectType,
		const int32& _Index,
		const class UItemLoadDataBlock* const  _SelectData,
		UButton* _CurrentSelectButt);
	//设置分解道具数量加成率
...
public:
	//创建材界面
	UWidget* WidgetCreate_ExchangeInit(const class UItemLoadDataBlock* const _Data, int32 _Index);
	//刷新界面
	void WidgetRefresh_ExchangeUpdate(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);

	//创建材界面
	UWidget* WidgetCreate_SplitMaterialInit(const class UItemLoadDataBlock* const _Data, int32 _Index);
	//刷新界面
	void WidgetRefresh_SplitMaterialUpdate(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);
	//设置材料属性
	void SetSplitMaterials(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);

	//创建材界面
	UWidget* WidgetCreate_CardInit(const class UItemLoadDataBlock* const _Data, int32 _Index);
	//刷新界面
	void WidgetRefresh_CardUpdate(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);
	//设置卡片属性
	void SetSplitCards(const class UItemLoadDataBlock* const _Data, int32 _Index, UWidget* _UWidget);
private:
	//界面标题
	UPROPERTY()
	...
	//当前选择的数据对象
	//UPROPERTY()
	//const UItemLoadDataBlock* const M_CurrentSelect = nullptr;
	//当前选择的按钮
	UPROPERTY()
...
private:
	...
	//（兑换）加载区
	UPROPERTY()
	UItemLoaderObject* M_Exchange_Loadding = nullptr;
	...
	//（材料）加载区
	UPROPERTY()
	UItemLoaderObject* M_SplitMaterial_Loadding = nullptr;
	...
	//（卡片）加载区
	UPROPERTY()
	UItemLoaderObject* M_Card_Loadding = nullptr;
};
