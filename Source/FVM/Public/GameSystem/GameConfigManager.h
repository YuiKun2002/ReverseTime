...
//玩家物品整理顺序
USTRUCT(BlueprintType)
struct FGamePlayerItemArrangeConfig {
...
public:
	...
	//设置角色物品整理
	UFUNCTION(BlueprintCallable)
	void SetPlayerItemArrangeConfig(const TMap<FName, FGamePlayerItemArrangeConfig>& Maps);
public:
	//获取当前登陆游戏的角色名称
	UFUNCTION(BlueprintCallable)
	...
	void GetPlayerItemArrangeConfig(TMap<FName, FGamePlayerItemArrangeConfig>& OutMaps);
public:
	//添加新的角色登陆名称
	UFUNCTION(BlueprintCallable)
...
public:
	...
	//物品整理
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "GameConfig")
	TMap<FName, FGamePlayerItemArrangeConfig> ArrangeConfig;
};
